...
impl TestContext {
    ...
    async fn create_workspace(&mut self) -> Result<(), String> {
        ...
        let workspace: serde_json::Value = response
            .json()
            .await
    ...
    async fn create_agent(&mut self) -> Result<(), String> {
        ...
        let agent: serde_json::Value = response
            .json()
            .await
    ...
    async fn monitor_task_logs(&self, duration_secs: u64) -> Result<Vec<String>, String> {
        ...
        let ws_url = if ws_token.is_empty() {
            ...
            format!(
                "ws://localhost:3000/api/tasks/{}/logs/stream?token={}",
                task_id, ws_token
            )
        };
    ...
    async fn cleanup(&self) -> Result<(), String> {
        ...
        println!(
            "Deleting Gitea repository: {}/{}",
            self.test_repo_owner, self.test_repo_name
        );
        self.gitea_client
...
async fn test_e2e_repository_setup() {
    ...
    ctx.setup_gitea_repository()
        .await
        .expect("Failed to create Gitea repository");
    ctx.setup_vibe_provider()
        .await
        .expect("Failed to create VibeRepo provider");
    ctx.sync_repositories()
        .await
        .expect("Failed to sync repositories");
    ctx.initialize_repository()
        .await
...
async fn test_e2e_complete_issue_to_pr_workflow() {
    ...
    let issue_body =
        "Create a simple hello_world() function that prints 'Hello, World!' to stdout.";
    ...
    println!("\n=== Phase 4: Wait for Completion ===");
    let task = ctx
        .wait_for_task_completion(600) // 10 minutes timeout
        .await
    ...
    assert!(
        pr.body.contains(&format!("#{}", issue.number)),
...
async fn test_e2e_websocket_log_monitoring() {
    ...
    assert!(
        !logs.is_empty(),
        "Should receive at least some log messages"
    );

    // Check for expected log patterns
    let has_log_messages = logs
        .iter()
        .any(|log| log.contains("\"type\":\"log\"") || log.contains("stream"));
