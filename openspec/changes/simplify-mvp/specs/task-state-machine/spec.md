...
**Reason**: The Assigned state is removed in the simplified state machine. Tasks go directly from Pending to Running.

**Migration**: Existing tasks in Assigned state will be converted to Pending state during migration. The assign_agent() method will be removed.

### Requirement: Failed state transitions

**Reason**: Retry functionality is removed. Failed tasks are now terminal states.

**Migration**: The retry_task() method will be removed. Users must create new tasks to retry failed operations.
